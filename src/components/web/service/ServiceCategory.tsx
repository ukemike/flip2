...
import { Clip, ArrowRight1, ArrowLeft, Location2 } from '../../../assets'
...
const Service = (props: any) => {
    ...
    React.useEffect(() => {
        if (props.query.minPrice || props.query.maxPrice) {
            setMinPrice(props.query.minPrice || '')
            setMaxPrice(props.query.maxPrice || '')
        } else {
            setMinPrice('')
            setMaxPrice('')
        }
    }, [props?.query?.minPrice, props?.query?.maxPrice])
    ...
    return (
        <>
            {props.loading || props.loadingFetchServices || props.services.length < 0 ?
